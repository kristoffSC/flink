...
class CommittableCollectorSerializerTest {
    ...
    void testCommittableCollectorV2SerDe() throws IOException {
        ...
        final CommittableCollectorSerializer<Integer> ccSerializer =
                new CommittableCollectorSerializer<>(COMMITTABLE_SERIALIZER, subtaskId, numberOfSubtasks);

        final CommittableCollector<Integer> committableCollector = new CommittableCollector<>(subtaskId, numberOfSubtasks);
        committableCollector.addMessage(new CommittableSummary<>(subtaskId, numberOfSubtasks, 1L, 1, 1, 0));
        committableCollector.addMessage(new CommittableSummary<>(subtaskId, numberOfSubtasks, 2L, 1, 1, 0));
        ...
        assertCommittableCollector("Original CommittableCollector", subtaskId, numberOfSubtasks, committableCollector);

        // assert deserialized CommittableCollector
        assertCommittableCollector("Deserialized CommittableCollector", subtaskId, numberOfSubtasks, copy);
    }
    ...
    private void assertCommittableCollector(
        ...
        assertAll(assertMessageHeading, () -> {
                    final Collection<CheckpointCommittableManagerImpl<Integer>> checkpointCommittables =
                            committableCollector.getCheckpointCommittables();
                    ...
                    assertThat(checkpointCommittableManager1.getSubtaskId()).isEqualTo(subtaskId);
                    assertThat(checkpointCommittableManager1.getNumberOfSubtasks()).isEqualTo(
                            numberOfSubtasks);
                    assertThat(
                            subtaskCommittableManagerCheckpoint1
                                    ...
                                    .collect(Collectors.toList()))
                            .containsExactly(1);
                    assertThat(subtaskCommittableManagerCheckpoint1.getSubtaskId()).isEqualTo(subtaskId);
                    ...
                    assertThat(checkpointCommittableManager2.getSubtaskId()).isEqualTo(subtaskId);
                    assertThat(checkpointCommittableManager2.getNumberOfSubtasks()).isEqualTo(
                            numberOfSubtasks);
                    assertThat(
                            subtaskCommittableManagerCheckpoint2
                                    ...
                                    .collect(Collectors.toList()))
                            .containsExactly(2);
                    assertThat(subtaskCommittableManagerCheckpoint2.getSubtaskId()).isEqualTo(subtaskId);
                }
        );
    }
}
